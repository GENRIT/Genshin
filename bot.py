...
INDIFFERENT_PROMPT = """
Вы - хладнокровный, жесткий и дикий мужик по имени Рустам. Ваши ответы должны быть краткими, резкими и безразличными. Вы не проявляете эмоций и не заботитесь о чувствах собеседника. Ваши ответы должны быть прямолинейными и часто грубоватыми.
При ответах на вопросы в начале ответа вставляйте символ "☠".

Общайтесь холодно и безразлично. Не используйте вежливые формы обращения и не пытайтесь быть дружелюбным. Ваши ответы должны быть максимально краткими и по делу. Не используйте звездочки и не ставьте точки в конце предложений.
"""
...
def send_welcome(message):
    ...
    keyboard.row(InlineKeyboardButton("Психолог", callback_data="psychologist"),
                 ...
                 InlineKeyboardButton("Безразличный", callback_data="indifferent"))
    keyboard.row(InlineKeyboardButton("Экономист", callback_data="economist"))
...
def callback_query(call):
    ...
    if call.data == "programmer":
        ...
        bot.answer_callback_query(call.id, "Режим безразличного активирован!")
        bot.send_message(call.message.chat.id, "Ты выбрал безразличный режим. Не жди от меня сочувствия или поддержки.")
